...
function calcCenter(p1, p2, relation) {
    var p = new Point();
    p.x = (p1.x + relation * p2.x) / (1 + relation);
    p.y = (p1.y + relation * p2.y) / (1 + relation);
    return p;
}
...
function render() {
    ...
    pointC = calcCenter(pointA, pointB, alpha.x / alpha.y);
    drawPoint(pointC.toContextCoord());

    var xFieldC = document.getElementById("xFieldC");
    var yFieldC = document.getElementById("yFieldC");

    xFieldC.value = pointC.x;
    yFieldC.value = pointC.y;
...
function inputValue(id) {
    ...
    switch (id) {
        ...
        case "yFieldAlp":
            {
                alpha.y = parseFloat(field.value);
                break;
            }
    }
}